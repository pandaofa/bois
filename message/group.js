...
const { getBuffer } = require('../Func/myfunc')
const { color, bgcolor } = require('../Func/color')
...
║┣❥ Cewe/Cowo?
╚════ ⸨ *ManikBot* ⸩ ═════`
       let beff = await getBuffer(`https://api.lolhuman.xyz/api/base/welcome?apikey=${setting.lolkey}&img1=${pp_user}&img2=${pp_grup}&background=https://telegra.ph/file/9c41931cada0cfe68b8b7.jpg&username=${encodeURI(anu_user)}&member=${memeg}&groupname= ${encodeURI(mdata.subject)}`)
const welcomebutton = await Manik.prepareMessage(mdata.id, beff, MessageType.image) // change for file type
const buttonsss = [
  {buttonId: '#infogroup', buttonText: {displayText: 'InfoGroup'}, type: 1},
